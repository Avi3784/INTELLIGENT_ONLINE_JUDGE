...
module.exports = (io) => {
  ...
  // Authenticate socket connections to the arena namespace
  arenaNamespace.use(async (socket, next) => {
    try {
      ...
      const user = await User.findById(decoded.id).select('username eloRating');

      if (!user) {
        return next(new Error('Authentication error: User not found'));
      }

      socket.user = { id: user._id, username: user.username, eloRating: user.eloRating };
      next();
    } catch (err) {
  ...
  arenaNamespace.on('connection', (socket) => {
    ...
    socket.on('join_queue', async (data) => {
      ...
      const eloRating = socket.user.eloRating;
      ...
      if (waitingPlayers.length >= 2) {
        ...
        const matchData = {
          ...
          players: [
            { userId: player1.userId, username: player1.username, socketId: player1.socketId, eloRating: player1.eloRating, progress: 'Started' },
            { userId: player2.userId, username: player2.username, socketId: player2.socketId, eloRating: player2.eloRating, progress: 'Started' }
          ],
          status: 'active'
        };
    ...
    socket.on('match_complete', async (data) => {
      ...
      if (match && match.status === 'active') {
        // Validate that the requesting user is part of this match
        const requestingPlayer = match.players.find(p => p.userId === userId);
        if (!requestingPlayer) {
          return;
        }
        ...
        if (winner && loser) {
          ...
          const winnerDoc = await User.findByIdAndUpdate(
            winner.userId,
            { $inc: { eloRating: 25, matchesWon: 1 } },
            { new: true }
          ).select('eloRating').exec();

          const loserDoc = await User.findByIdAndUpdate(
            loser.userId,
            { $inc: { eloRating: -25, matchesLost: 1 } },
            { new: true }
          ).select('eloRating').exec();

          const winnerElo = winnerDoc?.eloRating ?? (winner.eloRating + 25);

          arenaNamespace.to(winner.socketId).emit('match_over', {
            result: 'win',
            eloChange: 25,
            newElo: winnerElo
          });
          arenaNamespace.to(loser.socketId).emit('match_over', {
            result: 'loss',
            eloChange: -25,
            newElo: loserDoc?.eloRating ?? (loser.eloRating - 25)
          });
        }
      }
    });
        ...
        if (match.status === 'active') {
          ...
          if (disconnectedPlayer) {
            ...
            if (winner) {
              const winnerDoc = await User.findByIdAndUpdate(
                winner.userId,
                { $inc: { eloRating: 15, matchesWon: 1 } },
                { new: true }
              ).select('eloRating').catch(() => null);

              const winnerElo = winnerDoc?.eloRating ?? (winner.eloRating + 15);
